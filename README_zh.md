...
`Mask` / `MaskOf` 可能返回三类错误，均可用 `errors.Is` 判断：
...
- **非 `string` 字段带 tag**：静默跳过（不脱敏、不报错）；若该字段的类型是嵌套结构体，仍递归其内部。
- **未导出字段**：一律跳过（不脱敏、不递归）。
...
- **只处理 `string` 字段**：数字、时间等非字符串字段即使带 tag 也静默跳过，不会误伤，也不会报错。
...
- **未注册格式是错误而非警告**：格式名拼错会在首次调用时暴露（`字段 X: 格式 "y" 未注册`），多个错误用 `; ` 拼接；注册后下次调用自动修复。
- **注册格式使缓存失效**：`RegisterMask` 之后，之前脱敏过的类型下次调用会重建配置——这是特性，不是 bug。
- **并发安全**：`Mask` / `MaskOf` 可并发调用；`RegisterMask` 与脱敏可并发（写时复制注册表 + `atomic.Pointer` 缓存），但请把注册放在启动期。
- **幂等**：已掩掉的 `*` 保持不变，重复调用结果不变；生产上出口处脱敏一次即可。
